...
## 37. Phase 3B forward reference (2026-08-08)

AFENDA Phase 3B built the first real external/JSON transport boundary (`packages/contracts`, depending on `errors`/`time`/`money` and Zod 4, pinned exactly at `4.4.3`) and proved authoritative Money/Instant/CivilDate/AsOf values cross it exactly, including the full 2^53±1 and PostgreSQL-bigint-bounds corpus. It extended three existing Phase 3A controls to the 4-package graph rather than introducing new ones: SCC-03 (money-safety AST gate, now scoped to `packages/money` + `packages/contracts`, reassessed `partial` → `implemented` for the current topology) and SCC-05 (module-boundary graph, now 4 packages with a new `no-kernel-depends-on-contracts` rule and two new red fixtures) and SCC-24 (architecture control, confirmed to already cover the new package via its generic glob, zero violations). It also added three new mutation-kill red fixtures spanning the domain-to-transport boundary (SCC-18/V12) and materially strengthened V08's evidence (still `partial` — no database/API/queue boundary exists yet). SCC-12 (Hono runtime) was deliberately left `not-yet-built`: no Hono dependency was installed, per the phase brief. Findings 1–4 above are unchanged and not addressed by Phase 3B. Full detail, evidence, and honest state-change reasoning: `governance/PHASE_3B_CONTRACTS_REPORT.md`. This forward reference does not duplicate that report's content and is not itself new evidence.

---

## 38. Phase 3B.1 forward reference (2026-08-08)

AFENDA Phase 3B.1 repaired and hardened lint control evidence rather than adding new application surface. It found and fixed a genuine false-red defect in the lint gate's own red fixture: the prior fixture asserted only `errorCount > 0` against an invented, never-written-to-disk path, and that path's `errorCount: 1` was confirmed (both for the deliberately-bad content and for clean control content at the identical path) to be a `projectService` "file not found" parser failure (`ruleId: null, fatal: true`), not the intended `no-explicit-any`/`ban-ts-comment` rules firing. The fix — a real on-disk fixture, exact-ruleId assertions, an explicit rejection of any fatal/parser message as a substitute, and a same-path clean-source control — is generalized into a shared harness helper (`runLintRuleFixture` in `scripts/red.ts`) used by all rule-specific lint fixtures. The phase also enabled two previously-`'off'` unsafe-ingress rules, `@typescript-eslint/no-unsafe-assignment` and `@typescript-eslint/no-unsafe-argument` (`no-unsafe-member-access`/`no-unsafe-call` deliberately left `off`), fixed the one real violation this surfaced (`packages/errors/tests/result.test.ts`, via an explicit `unknown` annotation, not a cast/suppression), and added 3 new red fixtures: one proving each new rule fires on an untrusted `JSON.parse()` value, and a business-facing probe proving an untrusted-JSON-to-`Money` path fails lint without going through `packages/contracts`'s Zod schema while the sanctioned `decodeMoneyTransport(JSON.parse(...))` path lints clean (doctrine SEC-05). A `lint:dev` cached-lint script was added for local development only; `pnpm gate`/CI/Turbo/`pnpm red` remain on uncached `eslint .`. Red fixture count: 33 → 36. No control state changed (SCC-01/02/03 remain `implemented`; SCC-18/V08/V12 remain `partial`); only evidence was strengthened, and the historical false-red is recorded honestly rather than erased. The repair was subsequently mutation-verified at `a67ac88`: removing `no-unsafe-assignment` from `eslint.config.mjs` turned exactly the two dependent fixtures red with `got no-fail` (an absent rule, not a parser error), left the `no-unsafe-argument` fixture green, and made `pnpm red` exit `1` — so the fixtures are rule-scoped and a removed control cannot pass CI. Full detail: `governance/PHASE_3B1_LINT_REPORT.md`. This forward reference does not duplicate that report's content and is not itself new evidence.

---

## 39. Phase 3C forward reference (2026-08-08) — commit 1

AFENDA Phase 3C commit 1 established the first persistence boundary (`packages/db`, `db/migrations/0001_bootstrap.sql`) without ledger/business schema. It empirically confirmed node-postgres defaults return int8/numeric as strings, then installed owned parsers that keep that contract explicit and **throw on OID 790 (`money`)**. The lossy-parser red fixture mutates by *registering* `parseInt` for OID 20 (not by removing a registration — that would be false-red). Migrations are checksummed, forward-only, transactional by default with a per-file opt-out header; 0001 bootstraps role topology (`afenda_migrator` / `afenda_app`), migration history, and a `SECURITY DEFINER` recorder — no throwaway probe table. Single-client `withTransaction` is backed by an AST scan forbidding `pool.query` in `packages/db/src` (`pnpm gate` step 4i). Testcontainers PostgreSQL 18 evidence lives in `pnpm gate:db-integration`, deliberately separate from the fast `pnpm gate`. Control moves: SCC-08/09/11 → `implemented`; SCC-07 → `partial` (PG18 only); SCC-10/SCC-19 remain `not-yet-built` pending commit 2 (dual-major, PGlite structural gating, Kysely codegen from a single canonical source). Full detail: `governance/PHASE_3C_DB_REPORT.md`.
