...
Public API (`src/index.ts`): `ok`, `err`, `isOk`, `isErr`, `mapOk`, `mapErr`, `wrapErr`, `unwrapOr`, `matchResult`, `toPublicJson`, `normalizePublicErrorDetails`, `NON_FINITE_DETAIL_CANONICAL`, and the types `Result<T, C>`, `Ok<T>`, `Err<C>`, `ErrorShape<C>`, `PublicErrorJson<C>`, `PublicErrorDetails`, `PublicErrorDetailValue`, `ErrOptions`, `WrapErrOptions`, `ResultMatchers`. Semantic commitments (mapErr vs wrapErr, `-0` canonicalization, flat details) are also recorded in `packages/errors/README.md`.
...
- Detail records are always shallow-copied **and `Object.freeze`d** at construction and at the public projection (`err` / `mapErr` / `wrapErr` / `toPublicJson`), so caller or public-side top-level mutation cannot alias into a stored `ErrorShape`. Nested details are a type error (`tests/type-invalid/error-details-cannot-be-nested.ts`); shallow copy is therefore the complete aliasing contract for honest callers.
- Non-finite `number` detail scalars are normalized via exported `normalizePublicErrorDetails` / `NON_FINITE_DETAIL_CANONICAL` at `err` / `mapErr` / `wrapErr` / `toPublicJson`, and at `@afenda/contracts` failure decode. **Decided:** `NaN` → `'NaN'`, `±Infinity` → `'Infinity'`/`'-Infinity'`, and **`-0` → `'-0'`** — `JSON.stringify(-0)` would emit `0` and silently drop the sign; this is intentional determinism, not a quirk to reverse.
- **`wrapErr` vs `mapErr` (stack/STACK.md SC-05 — Specific errors):** `wrapErr` is the default provenance-preserving re-code path (entire original `ErrorShape` retained as `cause`). `mapErr` replaces the failure shape and is lossy unless the mapper spreads the original — unit-tested as an explicit contrast so agents cannot pick by vibes. Prefer `wrapErr` wherever cause chains / audit trails matter.
