...
export function buildAgentRulesData(authorityIndex: AuthorityIndex): AgentRulesData {
  ...
  return {
    ...
    current_layout: [
      ...
      { path: 'packages/errors, packages/time, packages/money', role: 'Phase 3A application kernel: canonical Result/error vocabulary, explicit temporal primitives (Instant/CivilDate/AsOf/Clock), and exact bigint-based money primitives (CurrencyCode/MinorUnits/Money/Rate/rounding). See governance/PHASE_3A_KERNEL_REPORT.md.' },
      { path: 'packages/contracts', role: 'Phase 3B external transport boundary: Zod 4 (exact pin 4.4.3) wire schemas and exact serialize/parse contracts for Money, Instant, CivilDate, AsOf and public-safe Result/Failure shapes. Depends on errors/time/money only; no Hono/API/OpenAPI dependency. See governance/PHASE_3B_CONTRACTS_REPORT.md.' },
      { path: 'packages/db, db/migrations/', role: 'Phase 3C persistence boundary: node-postgres pool/single-client transactions, exact type parsers (incl. forbidden OID 790 money), checksummed forward-only migrations bootstrapping role topology, Testcontainers PostgreSQL 18 integration lane via `pnpm gate:db-integration` (not folded into `pnpm gate`). No ledger/business schema yet. See governance/PHASE_3C_DB_REPORT.md.' },
      { path: 'package.json, tsconfig*.json, pnpm-workspace.yaml, .node-version, turbo.json, docker-compose.yml', role: 'Repository/tooling control-plane shell; Turborepo package tasks across errors/time/money/contracts/db; digest-pinned local Postgres 18 compose profile for development only.' },
    ],
